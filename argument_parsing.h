...
static inline
enum ArgParseError
parse_args_longstrings(ArgParser* parser, const StringView* longstrings, size_t count, enum ArgParseFlags);
...
static inline
intptr_t
check_for_early_out_args_longstrings(ArgParser* parser, const StringView* args, size_t args_count);
//
        ...
        for(size_t i = 0; i < keywords->count; i++){
            ...
            if(arg->dest.type == ARG_FLAG || arg->dest.type == ARG_BITFLAG){
                ...
                if(arg->altname1.text){
                    StringView tn = ArgTypeNames[arg->dest.type];
...
static inline
enum ArgParseError
parse_args_longstrings(ArgParser* parser, const StringView*args, size_t args_count, enum ArgParseFlags flags){
    ArgToParse* pos_arg = NULL;
    ArgToParse* past_the_end = NULL;
    if(parser->positional.count){
        pos_arg = &parser->positional.args[0];
